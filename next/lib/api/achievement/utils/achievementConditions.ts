// 또는 title로 매핑하는 방법 (더 유지보수하기 쉬움)
export const getConditionByTitle = (title: string) => {
    const titleConditions = {
        '첫 삽질': (history: any) => history.totalFurniture >= 1,
        '테트리스 마스터': (history: any) => history.totalFurniture >= 10,
        '미니멀리스트인 척': (history: any) => history.totalRooms >= 1 && history.totalFurniture <= 3,
