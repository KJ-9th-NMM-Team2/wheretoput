...
export function generateSaveValues(environmentState) {
  ...
  const wallValue = wallTexture !== 'color' && wallTexturePresets[wallTexture]
    ? wallTexture
    : wallColor;

  // 바닥: 텍스처가 color가 아니면 텍스처 키값, 아니면 색상값
  const floorValue = floorTexture !== 'color' && floorTexturePresets[floorTexture]
    ? floorTexture
    : floorColor;
 ...
 */
export function parseLoadedValues(wallValue, floorValue, wallTexturePresets, floorTexturePresets) {
  ...
  if (wallType.isColor) {
    ...
    result.wallColor = '#FFFFFF'; // 텍스처 사용 시 기본 색상
  } else {
    // 알 수 없는 값의 경우 - 안전하게 기본값 처리
    console.warn(`Unknown texture value: ${wallValue}, falling back to default`);
    result.wallColor = '#FFFFFF';
    result.wallTexture = 'color';
  }
  ...
  if (floorType.isColor) {
    ...
    result.floorColor = '#D2B48C'; // 텍스처 사용 시 기본 색상
  } else {
    // 알 수 없는 값의 경우 - 안전하게 기본값 처리
    console.warn(`Unknown texture value: ${floorValue}, falling back to default`);
    result.floorColor = '#D2B48C';
    result.floorTexture = 'color';
  }
