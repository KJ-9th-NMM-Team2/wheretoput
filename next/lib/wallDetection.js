...
export class WallDetector {
  ...
  areParallelLines(line1, line2, maxDistance, angleThreshold) {
    ...
    const angle2 =
      (Math.atan2(line2.y2 - line2.y1, line2.x2 - line2.x1) * 180) / Math.PI;
    ...
    // 겹침 구간 검증 추가
    // 수직선인지 수평선인지 판단
    const isVertical = Math.abs(angle1) > 45 && Math.abs(angle1) < 135;

    let overlap, minLength;

    if (isVertical) {
      // 수직선: Y 좌표 겹침 확인
      const y1Min = Math.min(line1.y1, line1.y2);
      const y1Max = Math.max(line1.y1, line1.y2);
      const y2Min = Math.min(line2.y1, line2.y2);
      const y2Max = Math.max(line2.y1, line2.y2);
      overlap = Math.max(0, Math.min(y1Max, y2Max) - Math.max(y1Min, y2Min));
      minLength = Math.min(y1Max - y1Min, y2Max - y2Min);
    } else {
      // 수평선: X 좌표 겹침 확인
      const x1Min = Math.min(line1.x1, line1.x2);
      const x1Max = Math.max(line1.x1, line1.x2);
      const x2Min = Math.min(line2.x1, line2.x2);
      const x2Max = Math.max(line2.x1, line2.x2);
      overlap = Math.max(0, Math.min(x1Max, x2Max) - Math.max(x1Min, x2Min));
      minLength = Math.min(x1Max - x1Min, x2Max - x2Min);
    }

    // 10% 이상 겹쳐야 병합
    return minLength > 0 ? overlap / minLength >= 0.1 : false;
  }
  ...
  calculateCenterLine(parallelLines) {
    ...
    const startX =
      centerX +
      (minProj -
        (centerX - mainLine.x1) * dirX -
        (centerY - mainLine.y1) * dirY) *
        dirX;
    const startY =
      centerY +
      (minProj -
  ...
  filterLines(lines, minLength = 80, angleThreshold = 5) {
    return lines.filter((line) => {
      ...
      return (
        normalizedAngle < angleThreshold ||
...
export function useWallDetection() {
  ...
  useEffect(() => {
    if (!detectorRef.current) {
      ...
      // OpenCV 로딩 상태 추적
      detectorRef.current.loadPromise
        .then(() => {
          setIsReady(true);
        })
        .catch(() => {
...
export async function createWallsFromFloorPlan(roomId = null) {
  ...
  if (roomId && !roomId.startsWith("temp_")) {
    try {
      console.log(
        `room_walls 테이블에서 방 ${roomId}의 벽 데이터를 가져옵니다.`
      );
      ...
      if (response.ok) {
        ...
        if (
          wallsResult.success &&
          wallsResult.walls &&
          wallsResult.walls.length > 0
        ) {
          console.log(
            `room_walls에서 ${wallsResult.wall_count}개의 벽을 가져왔습니다.`
          );
  ...
  try {
    ...
    console.log(
      `도면 데이터 로드됨: ${walls.length}개 벽, 축척: 1px = ${pixelToMmRatio}mm`
    );
    ...
    if (walls3D.length > 0) {
      const centerX =
        walls3D.reduce((sum, wall) => sum + wall.position[0], 0) /
        walls3D.length;
      const centerZ =
        walls3D.reduce((sum, wall) => sum + wall.position[2], 0) /
        walls3D.length;
      ...
      console.log(
        `벽 중심점 조정: centerX=${centerX.toFixed(
          2
        )}, centerZ=${centerZ.toFixed(2)}`
      );
    }
