...
export async function POST(request) {
  try {
    ...
    if (furniture.model_url) {
      ...
      // S3 URL인지 확인
      if (
        furniture.model_url.startsWith("https://") &&
        furniture.model_url.includes("s3")
      ) {
    ...
    const imageExtension =
      path.extname(new URL(furniture.image_url).pathname) || ".png";
    ...
    const cleanName = furniture.name
      .replace(/[^a-zA-Z0-9가-힣]/g, "")
      .substring(0, 20); // 특수문자 제거, 20자 제한
